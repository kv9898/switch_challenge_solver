...
app_ui = ui.page_fluid(
    ...
    ui.tags.head(
        ui.tags.style(".container-fluid {  max-width: 300px;}", type="text/css")
    ),
    ui.input_checkbox("shape_hide", "Hide Shapes"),
    ui.panel_conditional("!input.shape_hide", shapes("initial")),
    "Formula (press `/~ to ",
    ui.input_action_link("clear", "Clear"),
    "):",
    ui.input_text("fml", "", ""),
    ui.output_text_verbatim("text"),
    ui.panel_conditional("!input.shape_hide", shapes("final")),
)
        ...
        match type:
            ...
            case "empty":
                if initial() == "" or final() == "":
                    ...
                    return GetOutcome(initial(), final())
            case "short":
                return compute(fml)
            case "long":
